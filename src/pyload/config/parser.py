...
import semver
from pyload.utils.parse import boolean, bytesize, entries
...
from pyload.core.__about__ import __version_info__
...
class ConfigOption(object):
    ...
    _convert_map = {
        ...
        InputType.StrList:
            lambda l: [
                str(x) for x in l] if isiterable(l) else entries(l)
    }
...
class ConfigParser(ConfigSection):
    ...
    def _to_filevalue(self, value):
        return ','.join([str(v) for v in value]) if isiterable(value) else value
