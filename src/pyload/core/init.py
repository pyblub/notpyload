...
from pyload.core.__about__ import (__namespace__, __package__, __version__,
                        __version_info__)
from pyload.core.config import config_defaults, session_defaults
...
class Core(Process):
    ...
    def _init_consolelogger(self):
        if False and self.config.get('log', 'color_console') and ismodule('colorlog'):
            ...
            consoleform = colorlog.ColoredFormatter(
                fmt, datefmt, log_colors=primary_colors,
    ...
    def _init_managers(self):
        from .manager import (
            ...
            FileManager, InfoManager, PluginManager, TransferManager, RemoteManager)
    ...
    def _close_session(self):
        id = str(self.session.get('previous', 'id'))  # TODO: added str(), is previous still needed at all? maybe delete this and the next line
